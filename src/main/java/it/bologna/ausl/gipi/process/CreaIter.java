...
//        Long maxId = query(this.qIter.numeroIter.max());
        int i = query.select(this.qIter.numeroIter.max())
...
//                  .from(this.qIter);
    ...
    public Iter creaIter(IterParams iterParams) throws IOException {
        ...
        i.setAnnoIter(Calendar.getInstance().get(Calendar.YEAR));
        ...
        i.setNomeTitolo(p.getIdAziendaTipoProcedimento().getIdTitolo().getNomeTitolo());
        ...
        pc.setIdIter(i.getId());                                // ?? Ma qui non dovrei passere l'iter intero e non solo l'id?? forse manca fk?
        pc.setNomeTipoProcedimento(p.getIdAziendaTipoProcedimento().getIdTipoProcedimento().getNomeTipoProcedimento());
