    ...
    public void run() {
        ...
        if (service != null && service.getActive()) {
            ...
            for (Iter iter : iters) {
                ...
                // Calcolo e setto la nuova data di chiusura previdta
                if (iter.getDataAvvio() != null) {
                    Calendar c = Calendar.getInstance();
                    c.setTime(iter.getDataAvvio());

                    // manipulate date
                    if (iter.getDerogaDurata() != null) {
                        c.add(Calendar.DATE, iter.getDerogaDurata());                
                    }
                    if (iter.getGiorniSospensioneTrascorsi() != null) {
                        c.add(Calendar.DATE, iter.getGiorniSospensioneTrascorsi());
                    }
                    if (iter.getProcedimentoCache() != null && iter.getProcedimentoCache().getDurataMassimaProcedimento() != null) {
                        c.add(Calendar.DATE, iter.getProcedimentoCache().getDurataMassimaProcedimento());
                    }
